...
if (tasksAPI && usersAPI && tasksSite) {
  ...
  // Home page is simple here: a redirect to /site/tasks
  app.get("/", (req, res) => res.redirect("/site/tasks"));
  ...
  //app.delete("/site/tasks/:taskId?", tasksSite.deleteTask); // To include
  app.post("/site/tasks/:taskId?/delete", tasksSite.deleteTask); // To remove

  
  // update task by id
  app.put("/tasks/:taskId?", tasksAPI.updateTask);
  app.put("/site/tasks/:taskId?", tasksSite.updateTask);
  ...
  app.use("/tasks*", tasksAPI.errorHandler);
   ...
   // App listening...
  app.listen(PORT, () =>
    console.log(`Tasks app listening in http://${HOST}:${PORT}`),
  );
