...
describe("Testing Tasks Management API services with mock tasks data", () => {
    ...
    describe("Getting all tasks searching by string '4'", () => {
        // Arrange
        const taskId = "4";
    ...
    describe("Getting a specific task", () => {
        const taskId = 2;
        // Case of success
        it(`Getting task ${taskId} from user ${userId} should return a task`, async () => {
            // Arrange
            const expectedTask = {
                id: taskId,
                title: `Task ${taskId}`,
            ...
            // Act
            const task = await tasksServices.getTask(taskId, userToken);
            // Assert
            assert.deepStrictEqual(task, expectedTask)
        });

        // Case of error
        it("Getting task 1 from user ${userId} should return internal error", async () => {
            // Arrange
            const idTask = "1";
            // Act
            try {
    ...
    describe("Adding a task", () => {
        ...
        it("Adding a task for user ${userId}", async () => {
            // Arrange
            const expectedTask = Object.assign({}, newTask, {id: 11, userId: userId});
            // Act
            ...
            // Assert
            assert.deepStrictEqual(addedTask, expectedTask, "addTask should return a task of id 11");
